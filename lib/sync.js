  ...
  async.waterfall([
      ...
      function(cb){
        self
          ...
          console.log(' Sync :: %s missing files on local', self.diff.missRight.length);
...
SyncManager.prototype.generate = function(dir, callback){
  ...
  async.waterfall([
    ...
    function(cb){
      ...
      self.diff.missRight.forEach(function(df){
        lines.push(df);
      });

      var sh = lines.join('\n');
      fs.writeFile(path.join(out, 'missing-local'), sh, function(err){
        if (err){
          return cb(err);
        }
        console.log(' Sync :: missing-local generated');
        cb();
      });
    }
...
SyncManager.prototype.makeDiffSync = function(){
  ...
  self.diff.missRight = _.difference(self.currents, self.remote);
...
function _getRemote(schema, cb){
  ...
  console.log(' Ssh :: connecting to ' + schema.username+'@'+schema.host+':'+schema.port+'...');
